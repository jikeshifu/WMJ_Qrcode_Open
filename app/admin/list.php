...
if ($_GET['do'] == 'del') {
	...
	$result = httpPost('https://www.wmj.com.cn/apiv3/deldevice', $data_string);
    mlog("list:".$result);
...
if ($count) {
	...
	foreach ($list as $key => $value) {
		...
		$data = array("appid" => $config['appid'], "appsecret" => $config['appsecret'],'sn'=>$value['sn']);
        $data_string = json_encode($data);
	    $result = httpPost('https://www.wmj.com.cn/apiv3/devicestate', $data_string);
	    mlog("list:".$result);
		$result = json_decode(trim($result, "\xEF\xBB\xBF"), true);
		mlog("list:".$result['online']);
		if ($result['online']) {
